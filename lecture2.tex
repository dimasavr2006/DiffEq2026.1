...
\begin{example}
	...
	\end{equation*}
	\begin{equation*}
		\lambda_1 = -1, \quad \lambda_{2,3} = \frac{-3 \pm \sqrt{-3}}{2} = -\frac{3}{2} \pm i\frac{\sqrt{3}}{2} \implies \text{все корни имеют отрицательные вещественные части}
	\end{equation*}
	\begin{equation*}
		\Downarrow
	\end{equation*}
...
\begin{example}
	...
	\begin{equation*}
		\dv{V}{t} = \underbrace{2x}_{\pdv{V}{x}} \underbrace{(y - x^3)}_{\dv{x}{t}} + \underbrace{2y}_{\pdv{V}{y}} \underbrace{(-x - 3y^3)}_{\dv{y}{t}} = -2(x^4 + 3y^4) < 0 \implies \text{определенно-отрицательная функция}
	\end{equation*}
	$\Downarrow$
...
\begin{example}
	...
	Положим $b = 2a$, тогда
	\begin{equation*}
		\dv{V}{t} = -2a(x^2 + y^2) \le 0 \implies \text{при любом } a>0 \text{ и } b=2a \text{ функция } V = ax^2 + 2ay^2 \text{ будет опред.-положительной},
	\end{equation*}
...
\section{Устойчивость решений дифференциальных уравнений по отношению к изменению правых частей уравнения}
